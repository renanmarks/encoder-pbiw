...
#include "AnnulationBit.h"
#include "Interfaces/IAnnulationBit.h"
...
namespace PBIW
{
    ...
    rVex64PBIWInstruction::print(IPBIWPrinter& printer) const
    {
      ...
      AnnulBitsVector bits=getAnnulBits();
      AnnulBitsVector::iterator itBit;
          
      output.longWord<<=8;
      
      for(itBit = bits.begin();
          itBit < bits.end();
          itBit++) 
      {
          output.longWord|=(*itBit)->getValue();
          std::cout << (*itBit)->getValue() << std::endl;
      }
    ...
    void
    rVex64PBIWInstruction::createAnnulBits(int quantityAnnulBits)
    {
        AnnulationBit* bit;
        for(int i = 0; i < quantityAnnulBits; i++)
        {
            bit = new AnnulationBit(true);
            this->annulbits.push_back(bit);
        }
    ...
    void
    rVex64PBIWInstruction::printbits()
    {
        AnnulBitsVector bitsVector;
        AnnulBitsVector::const_iterator it;
        bitsVector = this->getAnnulBits();
//        bitsVector.at(2)->setValue(true);
        int count = 0;
        for(it = bitsVector.begin(); it < bitsVector.end(); it++)
//        for(int i = 0; i < bitsVector.size(); i++)
        {
            std::cout << "Bit: " << (*it)->getValue() << std::endl;
        }
    ...
    rVex64PBIWInstruction::hasOperandSlot(const Utils::OperandItem& operand) // O(1)
    {
      ...
      switch (operand.getType()) 
      {
        ...
        case Utils::OperandItem::BRSource:
          ...
          // Check if there is space in the 0-6 slot range;
          if (operands.size() < READFIELDS)
          {
            return true;
          }
